...
// Extremely basic YAML parser specifically designed for internal Axi Actions
...
static int parse_and_run_toon(const wchar_t *toon_file, const char *event_name) {
    ...
    // Simple YAML check for event binding: on: [event_name]
...
int ax_run_workflows(const wchar_t *root, const char *event_name) {
    ...
    if (hFind == INVALID_HANDLE_VALUE) {
        return AX_OK; // No YAML workflows found
    }
