...
node verify_compiler_routes() -> int:
    @C_Native
    """
    const char *compiler = g_argc > 1 ? g_argv[1] : "lang\\axiom_compiler.exe";
    ...
    _mkdir("network\\tmp\\axiom-wrap-sdd\\compiler-routes");
    for (i = 0; i < 5; ++i) {
        FILE *stream;
        snprintf(input, sizeof(input), "%s\\%s", root, relative_inputs[i]);
        snprintf(output, sizeof(output),
                 "%s\\network\\tmp\\axiom-wrap-sdd\\compiler-routes\\hello-%s.exe",
                 root, names[i]);
        snprintf(capture, sizeof(capture),
                 "%s\\network\\tmp\\axiom-wrap-sdd\\compiler-routes\\hello-%s.txt",
                 root, names[i]);
        DeleteFileA(output);
        DeleteFileA(capture);
    ...
    if (failures) exit(1);
    printf("PASS all base Axiom routes\n");
    """
