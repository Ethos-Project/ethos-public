...
static Axiom::DAG g_dag;
static FFI_SharedMemory g_shm_inst;

void Axiom_Initialize() {
    g_dag.nodes.clear();
    g_dag.edges.clear();
...
int Axiom_ParseSource(const char* sourceCode) { return 0; }
const char* Axiom_GetLastError() { return "No error"; }
...
// Drawing state
static std::shared_ptr<Axiom::Node> s_drawingWireFrom = nullptr;
static float s_mouseX = 0;
static float s_mouseY = 0;

void axiom_init_window(int width, int height) {
    s_width = width;
    s_height = height;

    if (g_dag.nodes.empty()) {
        Axiom_Initialize(); // init shm

        auto n1 = std::make_shared<Axiom::Node>();
        n1->title = "Trigger Event";
        n1->ui_x = 100; n1->ui_y = 200;

        auto n2 = std::make_shared<Axiom::Node>();
        ...
        auto w = std::make_shared<Axiom::Wire>();
        w->sourceNode = n1;
        w->targetNode = n2;
        w->style = Axiom::WireStyle::SOLID;
...
int axiom_render_frame(double deltaTime, RenderCommand* outCommands, int maxCommands) {
    int count = 0;
...
void axiom_handle_input(int key, int state) {}

void axiom_handle_mouse(float x, float y, int button, int state) {
        ...
        if (button == 0) {
            if (s_showContextMenu) {
                if (x >= s_contextMenuX && x <= s_contextMenuX + 200.0f &&
                    ...
                    if (s_contextMenuTargetNodeIdx == -1) {
                        if (clickedIdx == 0) {
                            auto n = std::make_shared<Axiom::Node>();
                            n->title = "New Node";
...
void axiom_handle_cursor(float x, float y) {
    s_mouseX = x; s_mouseY = y;
    if (s_isPanning) {
